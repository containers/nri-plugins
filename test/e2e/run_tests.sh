...
TESTS_DIR="$1"
# Where the output of the run goes. Given a directory, every topology writes
# into it; without one, each writes into a directory of its own under the
# current one. Discarding coverage data and reporting on it both cover the
# root, so that a run of several topologies is reported as a single run.
OUTPUT_DIR_ARG="${2:-}"
