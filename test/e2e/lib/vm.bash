...
vm-wait-pod-regexp() {
    ...
    local POD="$(vm-command-q "kubectl get pods $namespace_args | \
        awk '/${pod_regexp}/ && \$3 != \"Terminating\" { print \$1; exit }'")"
    if [ -z "$POD" ]; then
...
#   1: start from scratch, so that the report covers this run only
#   0: add to the data collected earlier (the default)
#
...
#
# Refuse anything else rather than taking it for a no: a value we do not
# recognize would leave the data of earlier runs in place, and the report of the
# run would cover more than the run without saying so.
...
vm-coverage-discard-collected() {
    ...
    # Delete our data files where we put them: both the name of a file and the
    # coverage directory holding it have to match. Either on its own is too
    # little, as the directory to discard under can be anywhere, the source tree
    # included, where a directory named coverage is just as likely to be a
    # package of ours, and a covmeta file outside one is not ours to remove.
...
vm-coverage-snapshot() {
    ...
    if [ -z "$id" ]; then
        # An uninstrumented build, one with the test APIs off, or a plugin the
        # test already terminated. Whatever such a plugin wrote to $GOCOVERDIR
        # on its way out is still collected.
