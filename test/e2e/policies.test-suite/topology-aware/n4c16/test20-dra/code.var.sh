...
wait-resourceslice-devices() {
    ...
    while [ "$elapsed" -lt "$timeout" ]; do
        vm-command "kubectl get resourceslices -o json | \
            jq -c '[.items[] | select(.spec.driver == \"nri.topology-aware.cpu\") | (.spec.devices // [])[] | {name, capacity: .capacity[\"nri/cpus\"].value, class: .attributes[\"nri/cpuClass\"].string, priority: .attributes[\"nri/pctPriority\"].string, allowMultipleAllocations, nodeAllocatableResources}]'"
