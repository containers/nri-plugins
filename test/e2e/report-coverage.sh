...
# a directory to report on can be anywhere, the source tree included, where
# such a directory is just as likely to be a package of ours or the output of
# the unit tests.
data_dirs=$(find "${dirs[@]}" -type f -name 'covmeta.*' -not -path "$outdir/*" \
                -printf '%h\n' | sort -u | paste -sd, -)
