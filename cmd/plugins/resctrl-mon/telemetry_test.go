...
func TestTelemetryPrometheusEndpoint(t *testing.T) {
	...
	assert.Contains(t, metrics, "# TYPE l3_llc_occupancy_bytes gauge\n")
	assert.Contains(t, metrics, "# TYPE l3_mbm_local_bytes_total counter\n")
	assert.Contains(t, metrics, "# TYPE l3_mbm_bytes_total counter\n")
	assert.Contains(t, metrics, "# TYPE perf_core_energy_joules_total counter\n")
	assert.Contains(t, metrics, "# TYPE perf_activity_farads_total counter\n")
...
func TestCachedGatherer_BoundsCollections(t *testing.T) {
	calls := 0
	c := &cachedGatherer{
	...
	assert.Equal(t, 1, calls, "gathers within ttl must reuse the cached result")

	c.last = c.last.Add(-c.ttl)
	_, err := c.Gather()
	require.NoError(t, err)
	assert.Equal(t, 2, calls, "a gather after ttl must collect again")
}
