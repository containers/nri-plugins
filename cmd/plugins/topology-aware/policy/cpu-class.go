...
func (p *policy) setReservedPoolCpuClass() {
	if p.cpuClasses == nil {
		return
	}
...
func (p *policy) resolveCpuClass(ctr cache.Container) (string, bool, error) {
	if p.cpuClasses == nil {
		return "", false, nil
	}

	class, isCtrScoped, err := cpuClassPreference(ctr)
	if err != nil {
		return "", false, err
	}
	...
	if !p.cpuClasses.IsKnownClass(class) {
		return "", false, fmt.Errorf("unknown CPU class %q", class)
	}
