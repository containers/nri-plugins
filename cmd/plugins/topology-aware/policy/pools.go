...
func (p *policy) buildPoolsByTopology() error {
	...
	// get assigned to all pools where the closest NUMA node(s) with CPUs
	// are assigned.
	//
	// Notes:
...
func (p *policy) enumeratePools() {
	...
	p.root.DepthFirst(func(n Node) {
		n.(*node).id = id
		id++
		log.Info("%s#%d: pool %s (depth %d)", indent("", 1), n.NodeID(), n.Name(), n.RootDistance())
...
func (p *policy) buildRootPool() {
	...
	if p.sys.SocketCount() > 1 {
		...
		log.Info("%s+ created virtual root pool", indent("", 1))
	} else {
		log.Info("%s- omitted virtual root pool (single socket HW)", indent("", 1))
	}
...
func (p *policy) buildSocketPool(socketID idset.ID, root Node) {
	...
	log.Info("%s+ created %spool %s", indent("", socket.depth),
		map[bool]string{true: "root "}[p.root == socket], socket.Name())
...
func (p *policy) buildDiePool(socketID, dieID idset.ID, socket Node) {
	...
	log.Info("%s+ created pool %s", indent("", die.depth), die.Name())
...
func (p *policy) buildNumaNodePool(socketID, nodeID idset.ID, parent Node) {
	if mi, _ := p.sys.Node(nodeID).MemoryInfo(); mi != nil && mi.MemTotal == 0 {
		// Notes:
		//   I am not sure what is the probability of such a setup in practice,
		//   but ideally we would only omit this node (and thus folding its CPUs
		//   to its parent), if all other nodes (or at at least multiple ones)
		//   with CPUs under the parent are at equal distance. Otherwise, if a
		//   single node is the closest to this one, it would be better to fold
		//   this node to that closest one.
		log.Info("%s- omitted pool %s%d (no memory attached)",
			indent("", parent.RootDistance()+1), "NUMA node #", nodeID)
		return
	}
	...
	log.Info("%s+ created pool %s", indent("", node.depth), node.Name())
}
...
func (p *policy) getCloseMemsWithCpu() map[idset.ID]idset.IDSet {
	...
	if len(close) > 0 {
		log.Info("CPU-less NUMA nodes: closest nodes with CPUs: %v", close)
	}
	...
	if len(reverse) > 0 {
		log.Info("NUMA nodes with CPUs: closest CPU-less nodes: %v", reverse)
	}
