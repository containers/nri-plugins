...
// resolveSysfsPath returns the sysfs path to use for the test, preferring an
// already-unpacked testdata/sysfs/server/sys directory if present, and
// otherwise unpacking testdata/sysfs.tar.bz2 into a fresh temp dir. dir is
// non-empty only when a temp dir was created and must be cleaned up by the
// caller.
func resolveSysfsPath(t *testing.T) (sysPath, dir string) {
	t.Helper()
	...
	return path.Join(dir, "sysfs", "server", "sys"), dir
}
...
func setupTestPolicy(t *testing.T) (*policy, string) {
	t.Helper()
	sysPath, dir := resolveSysfsPath(t)
...
// funcName returns the package-unqualified function name for pc.
func funcName(pc uintptr) string {
	full := runtime.FuncForPC(pc).Name()
	short := full[strings.LastIndex(full, "/")+1:]
	return short[strings.Index(short, ".")+1:]
}

// printCallers prints calling function names.
func printCallers(depth int) {
	for i := 2; i <= depth+1; i++ {
		pc, _, _, ok := runtime.Caller(i)
		if !ok {
			break
		}
		fmt.Printf(" %s()", funcName(pc))
	}
}
...
func free(p *policy, id string) error {
	if size, ok := mallocSizeByID[id]; ok {
		fmt.Printf("free(%dGB)", size>>30)
	} else {
		fmt.Printf("free(unknown id)")
	}
	if fmbtV >= 1 {
		fmt.Printf(" id=%s", id)
	}
...
func TestLibmemGofmbt(t *testing.T) {
	...
	allocNames := []string{"02GB", "04GB", "08GB", "16GB", "32GB"}
