...
func setupDRATestPolicyWithActivePCT(t *testing.T, hpCpus int) *policy {
	t.Helper()
	return setupDRATestPolicyWithActivePCTAndCfg(t, hpCpus, nil)
}

// setupDRATestPolicyWithActivePCTAndCfg is setupDRATestPolicyWithActivePCT
// with an additional cfg hook, letting a caller configure extra (non-HP)
// cpuClasses and SharedPoolCpuClass/ReservedPoolCpuClass. Added (instead of
// changing setupDRATestPolicyWithActivePCT's signature under its 4 existing
// callers) for TestReconfigureRefusalResyncsPhysicalCpuClassState, which
// needs reserved/shared/claim to be three *distinct* classes to discriminate
// one refusal-path re-sync step from another.
func setupDRATestPolicyWithActivePCTAndCfg(t *testing.T, hpCpus int, mutateCfg func(*cfgapi.Config)) *policy {
	t.Helper()
...
// addTestGrantWithClass mirrors addTestGrant's full pool-supply bookkeeping
// (pools_test.go) but additionally sets a cpuClass on the grant. Added as a
// sibling instead of a parameter on addTestGrant to avoid touching its two
// existing (class-less) callers.
func addTestGrantWithClass(t *testing.T, p *policy, pool Node, container cache.Container, exclusive cpuset.CPUSet, cpuClass string) Grant {
	t.Helper()

	g := newGrant(pool, container, cpuNormal, cpuClass, exclusive, 0, memoryDRAM, nil, 0)
...
// TestReconfigureRefusalResyncsPhysicalCpuClassState is the regression test
// for the refusal path's physical cpuClass re-sync: after a refused
// Reconfigure (live claim on a changed class), the restored *old* Handler's
// own cpuClass bookkeeping
// must have been forced to redo real work (Configure + reset + reserved +
// grant + claim reapplication) so ClassForCPU reports the correct class for
// every CPU category, discriminating one refusal-path re-sync step from
// another. It cannot prove the real-hardware bug itself is fixed (see this
// file's and topology-aware-policy.go's own comments on why: a separate,
// discarded Handler did the actual bad write, and the real mocked-SST state
// lives in an internal/ package this test can't import) — but it is a
// genuine regression test for the fix's own internal completeness, catching
// a future accidental omission of one of its steps.
func TestReconfigureRefusalResyncsPhysicalCpuClassState(t *testing.T) {
	p := setupDRATestPolicyWithActivePCTAndCfg(t, 4, func(cfg *cfgapi.Config) {
		cfg.CPUClasses = append(cfg.CPUClasses,
			&cfgapi.CPUClass{Name: "reserved-class"},
			&cfgapi.CPUClass{Name: "shared-class"},
			&cfgapi.CPUClass{Name: "grant-class"},
		)
		cfg.SharedPoolCpuClass = "shared-class"
		cfg.ReservedPoolCpuClass = "reserved-class"
		cfg.PinCPU = true // required for the grant-reapply step to run
		// Pin the reserved CPU explicitly, away from the mocked SST punit's
		// CPU range ("0-7", see sstOverrideJSON): the default quantity-based
		// reservation and the HP allocator's own CPU picking both prefer the
		// lowest-numbered available CPU, so without this they collide on
		// CPU 0 and the claim/reserved categories can't be discriminated.
		cfg.ReservedResources = cfgapi.Constraints{cfgapi.CPU: "cpuset:9"}
	})

	// A single-CPU claim, unlike the 2-CPU claim other tests in this file
	// use: the mocked SST punit's CPU range ("0-7", see sstOverrideJSON)
	// does not correspond to a single real NUMA pool in the "server" sysfs
	// fixture's actual (interleaved) topology, so a 2-CPU pick can span two
	// pools and fail allocateClaim's single-pool requirement below. A
	// single CPU always belongs to exactly one leaf pool.
	seedLiveHPClaim(t, p, types.UID("live-claim-1"), 1)

	// Physically mark the claim's CPUs with class "hp", mirroring a real
	// claim consumer (AllocateResources -> claimCPUsFromContainer ->
	// allocateClaim) -- seedLiveHPClaim alone only drives the DRA kubelet
	// plugin side (PrepareResourceClaims), which picks CPUs but never
	// paints a physical class.
	live := p.draPlugin.LiveClaimsLocked()
	var (
		claimUID  types.UID
	...
	for uid, allocs := range live {
		cpus, ccpus := classifyClaimCPUs(uid, allocs)
		if cpus.IsEmpty() {
			continue
		}
		claimUID, claimCPUs, classCPUs = uid, cpus, ccpus
		break
	}
	if claimCPUs.IsEmpty() {
		t.Fatal("test setup error: could not resolve the seeded claim's CPUs from LiveClaimsLocked()")
	}
	if err := p.allocateClaim(claimUID, claimCPUs, classCPUs); err != nil {
		t.Fatalf("allocateClaim() failed: %v", err)
	}
	claimCPU := claimCPUs.List()[0]

	if p.reserved.IsEmpty() {
		t.Fatal("test setup error: p.reserved is empty")
	}
	reservedCPU := p.reserved.List()[0]

	// Pick a spare allowed CPU, disjoint from the claim's and reserved
	// CPUs, for the grant.
	excluded := claimCPUs.Union(p.reserved)
	spare := p.allowed.Difference(p.isolated).Difference(excluded).List()
	if len(spare) < 2 {
		t.Fatalf("test setup error: need at least 2 spare allowed CPUs, got %d", len(spare))
	}
	grantCPU := spare[0]
	noneCPU := spare[1] // touched by none of claim/reserved/grant

	grantPool, err := p.poolForCPUs(cpuset.New(grantCPU))
	if err != nil {
		t.Fatalf("poolForCPUs(%d) failed: %v", grantCPU, err)
	}
	victim := &mockContainer{returnValueForGetID: "grant-victim"}
	addTestGrantWithClass(t, p, grantPool, victim, cpuset.New(grantCPU), "grant-class")
	...
	// Change the mocked hardware's HP CPU capacity, same as
	// TestReconfigureRefusesCpuClassAttrChangeWithLiveClaim, to trigger the
	// live-claim refusal path.
	t.Setenv("OVERRIDE_SST", sstOverrideJSON(2))

	newCfg := &cfgapi.Config{
		ReservedResources: cfgapi.Constraints{cfgapi.CPU: "cpuset:9"},
		...
		SharedPoolCpuClass:   "shared-class",
		ReservedPoolCpuClass: "reserved-class",
		PinCPU:               true,
		DRA:                  &cfgapi.TopologyAwareDRA{Enabled: true},
	}
	...
	if got := p.cpuClasses.ClassForCPU(reservedCPU); !strings.HasPrefix(got, "reserved-class") {
		t.Errorf("reserved CPU %d class = %q after refused Reconfigure, want prefix %q", reservedCPU, got, "reserved-class")
	}
	if got := p.cpuClasses.ClassForCPU(grantCPU); !strings.HasPrefix(got, "grant-class") {
		t.Errorf("grant CPU %d class = %q after refused Reconfigure, want prefix %q", grantCPU, got, "grant-class")
	}
	if got := p.cpuClasses.ClassForCPU(claimCPU); !strings.HasPrefix(got, "hp") {
		t.Errorf("claim CPU %d class = %q after refused Reconfigure, want prefix %q", claimCPU, got, "hp")
	}
	if got := p.cpuClasses.ClassForCPU(noneCPU); !strings.HasPrefix(got, "shared-class") {
		t.Errorf("unclaimed CPU %d class = %q after refused Reconfigure, want prefix %q", noneCPU, got, "shared-class")
	}
}
...
// TestChangedDRAClassesIgnoresCapacity verifies that equal capacity in both
// snapshots does not trigger a false "live-class change". Both Reconfigure
// snapshots are now produced by DRADevicesAtMaxCapacity (full GuaranteedHpCpus,
// independent of hpUsed), so when the underlying hardware has not changed,
// both snapshots carry the same capacity and changedDRAClasses must return
// empty.
func TestChangedDRAClassesIgnoresCapacity(t *testing.T) {
	className := "hp"
	devWithCapacity := func(name string, cpus int) resourceapi.Device {
		capStr := fmt.Sprintf("%d", cpus)
		cap := resource.MustParse(capStr)
		return resourceapi.Device{
			Name: name,
			Attributes: map[resourceapi.QualifiedName]resourceapi.DeviceAttribute{
				"nri/cpuClass": {StringValue: &className},
	...
	// Both snapshots reflect the same hardware (GuaranteedHpCpus=10), as
	// DRADevicesAtMaxCapacity would produce regardless of current hpUsed.
	oldDevices := []resourceapi.Device{devWithCapacity("hp-pkg0-punit0", 10)}
	newDevices := []resourceapi.Device{devWithCapacity("hp-pkg0-punit0", 10)}

	if changed := changedDRAClasses(oldDevices, newDevices); len(changed) != 0 {
		t.Errorf("changedDRAClasses() = %v, want empty (same hardware capacity)", changed)
	}
}
