...
func (p *balloons) newCompositeBalloon(blnDef *BalloonDef, confCpus bool, freeInstance int) (*Balloon, error) {
	...
	case cfgapi.ComponentCreationFirstMinBalloons:
		instComp := [][]int{} // (number-of-instances, component-index)
		for compIdx, comp := range blnDef.Components {
			...
			instComp = append(instComp, []int{numInstances, compIdx})
		}
		// sort, first by number-of-instances ascending, then by component-index ascending
		sort.Slice(instComp, func(i, j int) bool {
			return instComp[i][0] < instComp[j][0] ||
				(instComp[i][0] == instComp[j][0] && instComp[i][1] < instComp[j][1])
		})
		log.Debugf("instance-components of composite balloon %s: %v",
			blnDef.Name, instComp)
		// create only one component balloon for the component
		compIdx := instComp[0][1]
...
func (p *balloons) validateConfig(bpoptions *BalloonsOptions) error {
	...
	for _, blnDef := range bpoptions.BalloonDefs {
		...
		if blnDef.ComponentCreation != "" &&
			...
			blnDef.ComponentCreation != cfgapi.ComponentCreationFirstMinBalloons {
