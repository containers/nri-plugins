...
- [x] capture pre-claim `node.status.allocatable.cpu` baseline (as a `resource.Quantity`-comparable value, e.g. via `kubectl get node -o jsonpath` millicore form, so the post-claim comparison isn't a fragile string diff of `16` vs `16000m`) before creating the pod
...
- [ ] ⚠️ BLOCKED (architectural, not a test bug): assert `node.status.allocatable.cpu` is exactly 2 less than the pre-claim baseline — **this assertion cannot pass on any real cluster**. Implemented as `wait-allocatable-cpu-millis` (30s/2s poll) and run against the gated VM: `node.status.allocatable.cpu` stayed at `16` (baseline) throughout, both immediately after and a full 30s after the pod reached Ready with the claim allocated (ruling out propagation delay). Root-caused by reading `pkg/scheduler/framework/plugins/dynamicresources/nodeallocatabledynamicresources.go` in a local Kubernetes checkout (`/home/ed/git/k8s.io/kubernetes`, `upstream/release-1.36`): KEP-5517's `NodeAllocatableResourceMapping`/`AllocationMultiplier` accounting is consumed **only** by the scheduler's `DynamicResources` Filter/PreBind logic, operating on an in-memory `fwk.NodeInfo` cache to avoid overcommitting CPU when scheduling *additional* pods — no kubelet or control-plane code path (confirmed via `git grep NodeAllocatableResourceMapping -- pkg/kubelet/`: zero hits) ever writes the deduction back to the `Node` API object's `status.allocatable`, which kubelet sets purely from physical capacity/reservations. The mechanism's actual persisted signal is `pod.status.nodeAllocatableResourceClaimStatuses[]`, written by the scheduler at PreBind — confirmed present and correct on the real gated VM: `kubectl get pod dra-pod0 -o json | jq .status.nodeAllocatableResourceClaimStatuses` returned `[{"resourceClaimName":"hp-turbo-cpus","containers":["dra-pod0c0"],"resources":{"cpu":"2"}}]` while `kubectl get nodes -o json | jq '.items[0].status.allocatable.cpu'` simultaneously still read `"16"`. This matches this repo's own `docs/dra/landscape.md:31` ("`DynamicResources` plugin's Filter becomes authoritative for node-allocatable resource fit... In PreBind it writes `pod.status.nodeAllocatableResourceClaimStatuses[]`") — the more precise doc already got this right; it's `docs/dra/plan.md` (lines 52/104/205/322) and `docs/dra/design.md` that use the looser, inaccurate "deducted from `node.status.allocatable.cpu`" phrasing this plan's Technical Details #5 inherited. Fixing that phrasing is `docs/dra/plan.md`/`design.md` maintenance, out of this task's scope (Task 5 is `code.var.sh` only) — flagging for Task 6/7's attention. The assertion code itself is left in place, unmodified and genuinely failing (not softened/faked), per this task's explicit instruction to report a real architectural gap honestly rather than paper over it.
...
- [ ] run the complete test against the Task 1 gated VM; iterate until it passes end-to-end — iterated until the only remaining failure was the architectural allocatable-deduction gap above (no typos/quoting/log-pattern bugs remained); the complete test does **not** pass end-to-end (`Test verdict: FAIL`), so this item is left unchecked rather than falsely marked done
- [ ] run tests — must pass before task 6 — does not pass (see above); left unchecked, not carrying forward to Task 6 per this run's scope (Task 5 only)
