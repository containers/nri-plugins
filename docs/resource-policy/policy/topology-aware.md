...
- `cpuClasses`
   - Define a combination of CPU frequency, C-state, and turbo attributes.
     ...
     a CPU class. See the [CPU Tuning](#class-based-cpu-tuning)
...
CPU classes provide fine grained control over CPU behavior and power
management.

#### CPU class definitions.

Each CPU class in the configuration is an object with the following
properties:
...
The topology-aware policy allows assigning containers eligible for
exclusive CPU allocation to designated CPU classes. This happens
either by setting up a `defaultExclusiveCPUClass` in the configuration,
or by annotating the container to a particular CPU class using the
`cpu-class.resource-policy.nri.io` effective annotation base key.
...
devices. An eligible HP class can still be hidden from DRA by setting
`cpuClass.dra.publish: false`, which leaves it configurable via the normal
`cpu-class.resource-policy.nri.io` annotation but excludes it from device
publication; `dra.publish` defaults to `true` when unset. A `ResourceClaim`
can select on any of these attributes. For example, to request 2 CPUs from a
high-priority PCT class:
...
For more affinity options, see [Container Affinity and Anti-Affinity](#container-affinity-and-anti-affinity).
