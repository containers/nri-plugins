...
- **Mix and match workloads**: Flexibly isolates or combines
...
- **[Configuration Options](#3-configuration-options)**
  - **[Container-to-Balloon Assignment](#31-container-to-balloon-assignment)**
  - **[CPUs-to-Balloon Selection](#32-cpus-to-balloon-selection)**
  - **[Container Tuning](#33-container-tuning)**
  - **[CPU Tuning](#34-cpu-tuning)**
  - **[Built-in Balloon Types](#35-built-in-balloon-types)**
  - **[Toggle Pinning Memory, Memory Types, CPUs and Containers](#36-toggle-pinning-memory-memory-types-cpus-and-containers)**
  - **[Visiblity, Scheduling, Metrics, Logging](#37-visibility-scheduling-metrics-logging)**
- **[Cookbook](#4-cookbook)**
  - **[Latency-Critical Containers](#41-latency-critical-containers)**
  - **[Maximum Memory Bandwidth Containers](#42-maximum-memory-bandwidth-containers)**
  - **[Workload-Aware Hyperthread Sharing](#43-workload-aware-hyperthread-sharing)**
...
helm install my-balloons nri-plugins/nri-resource-policy-balloons \
...
### 3.1. Container-to-Balloon Assignment
...
- First matching balloon type in the list is used.
...
1. Balloon type effective for the container is specified by its pod
   annotation (see Section 3.3). This skips matching in user-defined
   and built-in balloon types.
2. Implicit built-in reserved balloon type matches the container (see
   Section 3.5).
3. The first balloon type in the `balloonTypes` list where a
   `matchExpression` or a `namespace` matches the container.
4. Implicit built-in default balloon type matches the container (see
   Section 3.5).
5. If no match, creating the container fails.

**Composite balloons with `componentCreation: balance-balloons`:**
- When a balloon type has `components` list, it becomes a composite balloon
- `componentCreation: balance-balloons` creates only one component -
  the one whose balloon type has the fewest instances
- Use this to alternate container assignments across multiple balloon types
...
When a container matches a balloon type, these options determine which
instance of that type receives the container.
...
- Containers with the same evaluated value go to the same balloon.
...
- Device paths like `/sys/class/net/eth0` or `/sys/class/drm/card0`.
...
setting if not specified, default: `false`)
...
- Recommendation: use `loads` and `loadClasses` for better controls in
...
- `name` (string): Load class identifier (referenced in balloon types'
  `loads` lists).
- `level` (string): Hardware topology level affected: `"core"` or
  `"l2cache"`.
...
3. When allocating CPUs for another balloon with the same load class,
   allocator avoids loaded topology levels.
...
#### CPU Requirements for Components (Composite Balloons)

For balloons with diverse CPU requirements, use composite balloons
where each component specifies different properties:
...
Each CPU class (keyed by name) defines:
...
- Uses CPUs specified in `reservedResources.cpu`. If a `cpuset` is
  ...
  balloon, and correspondingly avoided when by other
  balloons. Reserved balloon can inflate beyond this cpuset if
  required by its containers.

**`reservedResources`** (policy-level configuration):
- `cpu` (string): CPUs for reserved balloon.
  - Preferred CPUs: `"cpuset:0,48"` uses CPU 0 and 48.
  - Quantity: `"2000m"` or `"2"` uses any 2 CPUs.
...
### 3.6. Toggle Pinning Memory, Memory Types, CPUs and Containers
...
- Warning: Pinning may cause OOM kills if local NUMA nodes have
  insufficient memory.

**`memoryTypes`** (list, per balloon type):
...
### 3.7. Visibility, Scheduling, Metrics, Logging
...
# Port forwarding
kubectl port-forward -n kube-system deployment/nri-resource-policy-balloons 8891:8891
...
### 4.1. Latency-Critical Containers
...
### 4.2. Maximum Memory Bandwidth Containers
...
### 4.3. Workload-Aware Hyperthread Sharing
...
**Solution**: Verify the configuration object exists, is valid and is in the correct namespace. If necessary, delete the policy pod and let the DaemonSet re-create it.
```sh
kubectl get balloonspolicies.config.nri -n kube-system
kubectl describe balloonspolicies.config.nri/default -n kube-system
```
