...
type Config struct {
	...
	// SharedCpuClass controls how CPUs are reconfigured when they are returned
	// from exclusive use to a shared pool. If any CPU classes are defined
	// a shared CPU class must be specified.
	// +optional
	SharedCpuClass string `json:"sharedCPUClass,omitempty"`
	// ReservedCpuClass controls how CPUs are reconfigured when they are assigned
	// to the reserved pool. If no reserved CPU class is designated, the shared
	// CPU class will be used for the reserved pool.
	// +optional
	ReservedCpuClass string `json:"reservedCPUClass,omitempty"`
...
func (c *Config) Validate() error {
	...
	if len(c.CPUClasses) > 0 {
		...
		if _, ok := cc[c.SharedCpuClass]; !ok {
			errs = append(errs, fmt.Errorf("unknown shared CPU class %q", c.SharedCpuClass))
		}

		if c.ReservedCpuClass != "" {
			if _, ok := cc[c.ReservedCpuClass]; !ok {
				errs = append(errs, fmt.Errorf("unknown reserved CPU class %q",
					c.ReservedCpuClass))
			}
		} else {
			c.ReservedCpuClass = c.SharedCpuClass
		}
