...
func TestValidateCPUClassesForDRA(t *testing.T) {
	tests := []struct {
		...
		{
			// Regression: two published LP classes must not trigger a tier-overcommit
			// error. DRADevices uses hpOnly=true, so LP classes produce no devices
			// and cannot overcommit any tier.
			name: "two managed LP classes, both published — ok (hpOnly skips them)",
			classes: []*policyapi.CPUClass{
				{Name: "lp1", PctPriority: "low"},
				{Name: "lp2", PctPriority: "low"},
			},
			wantErr: false,
		},
		{
		...
		{
			name: "two assoc-only classes with same SstClosID, sharedCounters=false — error (same CLOS, can both be HP-published)",
			classes: []*policyapi.CPUClass{
...
func TestBuildDRADevices(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			devices := buildDRADevices(driverName, tc.classes, tc.punits, isHPFn, true)
