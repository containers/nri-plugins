...
// TestResmgrStopNilNriDoesNotPanic verifies that Stop() remains safe when the
// NRI plugin was never started, so startup failures before m.nri.start() do not
// end up dereferencing a nil stub.
func TestResmgrStopNilNriDoesNotPanic(t *testing.T) {
	m := &resmgr{nri: &nriPlugin{}}
	assert.NotPanics(t, func() {
		m.Stop()
	})
}

// TestNriPluginStopNilStubDoesNotPanic verifies that shutdown is safe when the
// plugin exists but its stub was never created during a failed startup path.
func TestNriPluginStopNilStubDoesNotPanic(t *testing.T) {
	p := &nriPlugin{}
	assert.NotPanics(t, func() {
		p.stop()
	})
...
func TestPostReconfigureCalledOnSuccess(t *testing.T) {
	...
	require.NoError(t, m.postReconfigure(nil))
...
func TestPostReconfigureNotCalledOnReconfigureError(t *testing.T) {
	...
	require.NoError(t, m.postReconfigure(errors.New("reconfigure failed")))
...
func TestPostReconfigureReturnsPolicyError(t *testing.T) {
	fp := &fakePolicy{postReconfigureErr: assert.AnError}
	m := &resmgr{policy: fp}

	assert.ErrorIs(t, m.postReconfigure(nil), assert.AnError)
	assert.Equal(t, 1, fp.postReconfigureCalled)
}
