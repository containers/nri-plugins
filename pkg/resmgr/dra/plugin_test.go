...
func TestHandleError_RecoverableLogsWarn(t *testing.T) {
	...
	// Must not panic.
	p.HandleError(t.Context(), recoverableErr, "publish failed")
}
...
func TestHandleError_FatalLogsError(t *testing.T) {
	...
	// Must not panic.
	p.HandleError(t.Context(), fatalErr, "fatal error encountered")
}
...
func TestPublishResources_NilHelper(t *testing.T) {
	...
	err = p.PublishResources(t.Context())
	if err == nil {
...
// TestTriggerRepublish_EnqueuesSignal verifies that TriggerRepublish sends
// exactly one signal on the buffered republishCh when the channel is empty.
func TestTriggerRepublish_EnqueuesSignal(t *testing.T) {
	p, err := New("test-driver", validDeps())
	if err != nil {
		t.Fatalf("New() unexpected error: %v", err)
	}
	...
	select {
	case <-p.republishCh:
		t.Fatal("unexpected signal in republishCh before TriggerRepublish")
	default:
	}
	...
	case <-p.republishCh:
	default:
		t.Fatal("no signal in republishCh after TriggerRepublish")
	}
}

// TestTriggerRepublish_Idempotent verifies that multiple TriggerRepublish
// calls while a publish is already pending enqueue exactly one signal (no
// blocking, no panic, no second entry).
func TestTriggerRepublish_Idempotent(t *testing.T) {
	p, err := New("test-driver", validDeps())
	if err != nil {
		t.Fatalf("New() unexpected error: %v", err)
	}
	p.TriggerRepublish()
	p.TriggerRepublish() // second call must not block or add a second signal
	...
	for {
		...
		default:
			if count != 1 {
				t.Errorf("republishCh had %d signals after two TriggerRepublish calls, want 1", count)
			}
			return
		}
	}
...
func TestPrepare_Idempotent_SpecMissing_PartialCorruptCPU(t *testing.T) {
	...
	// Two stored allocs: first is valid, second has an unparsable CPU string.
...
// TestPrepare_ClaimStoreSaveFailure verifies that a ClaimStore.Save failure
// after a successful CDI write and CPU pick rolls back symmetrically: the
// just-written CDI spec is removed, the picked CPUs are released, and the
// claim is not left in p.claims — otherwise a restart would lose durable
...
func TestPrepare_ClaimStoreSaveFailure(t *testing.T) {
	...
	r := result[uid]
	if r.Err == nil {
		t.Fatal("expected per-claim error from ClaimStore.Save failure, got nil (PrepareResult{} instead)")
	}
	if !errors.Is(r.Err, saveErr) {
		t.Errorf("PrepareResult.Err = %v, want to wrap saveErr", r.Err)
	}
	// The just-written CDI spec must be removed.
	if len(cdiW.removed) != 1 || cdiW.removed[0] != uid {
		t.Errorf("CDIWriter.RemoveClaim called for %v, want [%v]", cdiW.removed, uid)
	}
	// The picked CPUs must be released.
	if len(alloc.releases) == 0 {
		t.Error("expected ReleaseHpCpus to be called on ClaimStore.Save failure")
	}
	// The claim must not be left set in p.claims.
	if _, ok := p.claims[uid]; ok {
		t.Error("claim stored in p.claims after ClaimStore.Save failure (should have been rolled back)")
	}
}
...
// TestStart_Reconciliation verifies that persisted claims are re-accounted from
// durable state even when the CDI spec file is missing on disk. Missing CDI is
// not treated as a proof of staleness; only an actual Unprepare removes a claim
// from the store. Orphan CDI specs (not tracked in the claim store) are still
// swept and removed.
func TestStart_Reconciliation(t *testing.T) {
	...
	// Both claims must remain in p.claims — staleUID is kept even though its
	// CDI spec is absent; PrepareResourceClaims will recreate it.
	if _, ok := p.claims[liveUID]; !ok {
		t.Error("liveUID missing from p.claims after Start")
	}
	if _, ok := p.claims[staleUID]; !ok {
		t.Error("staleUID missing from p.claims after Start — missing CDI spec must not imply stale claim")
	}

	// Both persisted claims are re-accounted from durable state.
	if len(alloc.accounts) != 2 {
		t.Errorf("AccountHpCpus called %d times, want 2", len(alloc.accounts))
	}

	// No claim-store writes happen when we keep persisted claims.
	if store.saved != 0 {
		t.Errorf("ClaimStore.Save called %d times, want 0", store.saved)
	}
	...
	// staleUID should NOT be in cdiW.removed (it is a live persisted claim).
	for _, uid := range cdiW.removed {
		if uid == staleUID {
...
// TestStart_ClaimStoreLoadError verifies that a ClaimStore.Load error during
// Start's reconciliation causes Start to return a non-nil error and does not
// proceed to the orphan sweep with an empty claim map — otherwise a transient
// Load failure would cause the orphan sweep to remove CDI specs for claims
// whose consuming containers are still running.
func TestStart_ClaimStoreLoadError(t *testing.T) {
	liveUID := types.UID("uid-live-on-disk")

	alloc := &trackingClaimAllocator{isHP: true}
	cdiW := &startTestCDIWriter{
		existsByUID: map[types.UID]bool{liveUID: true},
		// ListClaims reports liveUID's CDI spec still exists on disk.
		listResult: []types.UID{liveUID},
	}
	store := &preloadedClaimStore{
		loadErr: errors.New("claim store load failed"),
	}
	...
	err = p.Start(ctx)
	if err == nil {
		t.Fatal("Start() expected error on ClaimStore.Load failure, got nil")
	}
	defer p.Stop()

	// The orphan sweep must not have run: liveUID's CDI spec must not have
	// been removed even though p.claims would have been empty.
	for _, uid := range cdiW.removed {
		if uid == liveUID {
			t.Errorf("orphan sweep removed CDI spec for %v after a ClaimStore.Load failure — live claim consumer's CDI spec destroyed", liveUID)
		}
	}
	if len(cdiW.removed) != 0 {
		t.Errorf("CDIWriter.RemoveClaim called %d time(s), want 0 (orphan sweep must be skipped on Load failure)", len(cdiW.removed))
	}
}
...
func TestStart_InactiveAllocator(t *testing.T) {
	...
	alloc := &trackingClaimAllocator{accountErr: fmt.Errorf("test: %w", cpuclass.ErrAllocatorInactive), isHP: true}
...
// TestStart_AccountHpCpus_FatalError verifies that Start fails when
// AccountHpCpus returns an error that does not wrap ErrAllocatorInactive
// (e.g. punit not found due to topology change). Registering in that state
// would leave CPUs unaccounted and allow double-allocation.
func TestStart_AccountHpCpus_FatalError(t *testing.T) {
	uid1 := types.UID("uid-1")

	alloc := &trackingClaimAllocator{accountErr: errors.New("pct: AccountHpCpus: punit (pkg=0, punit=0) not found"), isHP: true}

	cdiW := &startTestCDIWriter{
		existsByUID: map[types.UID]bool{uid1: true},
		listResult:  []types.UID{uid1},
	}
	...
	if err := p.Start(ctx); err == nil {
		p.Stop()
		t.Fatal("Start() expected error for non-inactive AccountHpCpus failure, got nil")
	}
}
