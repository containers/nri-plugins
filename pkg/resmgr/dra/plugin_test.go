...
// our node, with one slice in it, holding the devices of the policy exactly as
// the policy described them.
func TestDriverResources(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if devices := pool.Slices[0].Devices; !reflect.DeepEqual(devices, tc.devices) {
