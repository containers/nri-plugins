...
type Plugin struct {
	mu          sync.Mutex // mu guards helper, draCancel, draLoopDone
	driverName  string
	deps        Deps
	helper      *kubeletplugin.Helper
	draCancel   context.CancelFunc
	draLoopDone chan struct{}
	claims      map[types.UID]*ClaimState
	republishCh chan struct{}         // signals the republisherLoop to re-run PublishResources
	handleErr   atomic.Pointer[error] // set on non-recoverable HandleError; checked by PublishResources
}
			...
			result[uid] = func() kubeletplugin.PrepareResult {
				...
				if _, exists := p.claims[uid]; exists {
					...
					if len(cdiDevices) != len(p.claims[uid].Allocs) {
						...
						// start failures.
						return kubeletplugin.PrepareResult{Err: fmt.Errorf("dra plugin: idempotent re-write: only %d of %d CDI devices rebuilt from stored state (stored CPUs unparsable)", len(cdiDevices), len(p.claims[uid].Allocs))}
					}
					if len(cdiDevices) > 0 {
				...
				// Allocate CPUs for each filtered result.
				...
				for i, r := range filtered {
					...
					// claim's results to fit a single leaf pool.
					if len(pickedAllocs) > 0 {
						first := pickedAllocs[0]
						if attrs.PkgID != first.PkgID || attrs.PunitID != first.PunitID {
							p.rollbackPicks(pickedAllocs)
				...
				if saveErr := p.deps.ClaimStore.Save(p.claims); saveErr != nil {
					// Roll back symmetrically: the CDI spec and CPU picks
					// are already live, so leaving the claim "prepared"
					// here would lose durable state on a restart.
					delete(p.claims, uid)
					if removeErr := p.deps.CDIWriter.RemoveClaim(uid); removeErr != nil {
						p.deps.Logger.Warnf("dra plugin: PrepareResourceClaims: rollback after ClaimStore.Save failure: RemoveClaim: %v", removeErr)
					}
					p.rollbackPicks(pickedAllocs)
					return kubeletplugin.PrepareResult{Err: fmt.Errorf("dra plugin: ClaimStore.Save: %w", saveErr)}
				}
...
// is called, live claims are kept with a warning. If AccountHpCpus fails for
// any other reason (e.g. punit not found due to topology change), Start returns
// an error rather than registering the driver in an unsafe state.
func (p *Plugin) Start(ctx context.Context) error {
	...
	p.deps.WithLock(func() {
		...
		if loadErr != nil {
			// A Load failure must not be treated as "no claims": that would
			// let the orphan sweep below remove CDI specs that still belong
			// to live claim consumers. Fail startup outright instead.
			startErr = fmt.Errorf("dra plugin: Start: ClaimStore.Load: %w", loadErr)
			return
		}
		if loaded != nil {
			p.claims = loaded
		...
		// Re-account every persisted claim from durable state. A missing CDI spec
		// on disk does not by itself prove the claim is stale: kubelet commonly
		// clears /var/run/cdi on reboot, while the claim store is the durable
		// source of truth for active claims. Keep persisted claims accounted and
		// let the next Prepare re-create a missing CDI spec if needed.
		for uid, cs := range p.claims {
			if !p.deps.CDIWriter.ClaimSpecExists(uid) {
				p.deps.Logger.Warnf("dra plugin: Start: claim %s has no CDI spec on disk; keeping persisted claim and re-accounting from durable state", uid)
			}
			for _, alloc := range cs.Allocs {
				if alloc.ClassName != "" && !p.deps.ClaimAllocator.IsHPClass(alloc.ClassName) {
				...
				if err := p.deps.ClaimAllocator.AccountHpCpus(alloc.PkgID, alloc.PunitID, cpus); err != nil {
					if errors.Is(err, cpuclass.ErrAllocatorInactive) {
						p.deps.Logger.Warnf("dra plugin: Start: claim %s device %s: AccountHpCpus: %v (allocator inactive — keeping claim, container may be running)", uid, alloc.Device, err)
					} else {
						startErr = fmt.Errorf("dra plugin: Start: claim %s device %s: AccountHpCpus: %w", uid, alloc.Device, err)
						return
					}
				}
			}
	...
	draCtx, draCancel := context.WithCancel(ctx)
	loopDone := make(chan struct{})
	p.helper = helper
	p.draCancel = draCancel
	p.draLoopDone = loopDone
	p.mu.Unlock()
	go func() {
		defer close(loopDone)
		p.republisherLoop(draCtx)
	}()
	return nil
}

// TriggerRepublish enqueues a request to re-run PublishResources. It is
// non-blocking and safe to call while holding the resmgr lock: if a
// republish is already pending, the extra request is dropped (the pending
// one covers it). The actual publish runs in republisherLoop, outside
// any lock.
func (p *Plugin) TriggerRepublish() {
	select {
	case p.republishCh <- struct{}{}:
	default:
		// A republish is already pending; no need to enqueue another.
	}
}

// republisherLoop runs in a goroutine started by Start(). It drains
// republishCh and calls PublishResources for each signal, stopping when
// ctx is cancelled (i.e. when the plugin's draCtx is cancelled by Stop()).
// On a publication failure the loop re-enqueues itself with exponential
// backoff (1s → 2s → … → 60s) so that the ResourceSlice is eventually
// brought back in sync without hammering the API on persistent errors.
func (p *Plugin) republisherLoop(ctx context.Context) {
	const (
	...
	for {
		...
		case <-p.republishCh:
			if err := p.PublishResources(ctx); err != nil {
				p.deps.Logger.Warnf("dra plugin: background republish: %v (retrying in %s)", err, backoff)
				select {
				case <-ctx.Done():
					return
...
// Recoverable errors trigger a re-publish so the republisherLoop retries.
// Non-recoverable errors are stored so that the next PublishResources call
// returns them, causing the republisherLoop to enter its backoff path.
func (p *Plugin) HandleError(_ context.Context, err error, msg string) {
	if errors.Is(err, kubeletplugin.ErrRecoverable) {
		...
		e := fmt.Errorf("dra plugin: resource slice controller: %w", err)
		p.handleErr.Store(&e)
	}
}
