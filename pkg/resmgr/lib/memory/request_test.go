	...
	for _, tc := range []*testCase{
		{
			...
			req:    NewRequest("0", 0, 1),
			result: "besteffort workload<ID:#0>",
		},
		{
			name:   "burstable request #1 of 1M",
			req:    NewRequest("1", 1024*1024, 1),
			result: "burstable workload<ID:#1, size 1M>",
		},
		{
			name: "guaranteed request #2 of 2.75M",
			req: NewRequest("2", 2*1024*1024+768*1024, 1,
				WithName("default/busybox/ctr0"),
				WithQosClass("Guaranteed"),
			),
			result: "guaranteed workload<default/busybox/ctr0, size 2.75M>",
		},
		{
			name: "preserved workload #3 of 1G",
			req: NewRequest("2", 1024*1024*1024, 1,
				WithName("preserved container"),
				WithInertia(Preserved),
			),
			result: "preserved workload<preserved container, size 1G>",
		},
		{
			name:   "memory reservation #3 of 1.25G",
			req:    ReservedMemory(1024*1024*1024+256*1024*1024, 1, WithName("test")),
			result: "memory reservation<test, size 1.25G>",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
