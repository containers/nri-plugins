...
// it is the set of nodes from which memory is be allocated from. The type
...
// preference results in a failed allocation. The request also has a so
// so called inertia. This tells the allocator how easily or eagerly it
// should later move the allocation to other memory zones, when some zone
// gets runs out of memory.
//
...
//
// Allocator can be customized in multiple ways. The simplest and most
...
// set of nodes added to a zone during expansion.
//
// Another, more direct way to customize Allocator is to explicitly set it
// up with custom functions for node expansion and/or overflow resolution.
// These custom functions are called whenever node expansion is necessary
// (initial zone selection, overflow resolution), or when oversubscription
// has been detected.
package libmem
