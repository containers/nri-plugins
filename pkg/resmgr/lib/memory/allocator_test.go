	...
	for _, c := range []*CustomFunctions{nil, customDefault} {
		...
		for _, tc := range []*testCase{
			...
			t.Run(name, func(t *testing.T) {
				...
				case tc.strict:
					req = ContainerWithStrictTypes(tc.id, name, qos, tc.limit, tc.affinity, tc.types)
				default:
					req = ContainerWithTypes(tc.id, name, qos, tc.limit, tc.affinity, tc.types)
				}
...
func TestStrictAllocation(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.strict {
				req = ContainerWithStrictTypes(tc.id, tc.name, tc.qosClass, tc.limit, tc.affinity, tc.types)
			} else {
				req = ContainerWithTypes(tc.id, tc.name, tc.qosClass, tc.limit, tc.affinity, tc.types)
			}
