...
| `openshift.grant-scc`    | false                                                                                                                         | on OpenShift, grant the plugin ServiceAccount an SCC so its pods pass admission (`privileged` when `allowPCT`, else a dedicated least-privilege SCC) |
